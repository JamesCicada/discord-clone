...
export const ServerChannel = ({channel, server,role}:ServerChannelProps) =>{
    ...
    const Icon = typeIconsMap[channel.type]
    ...
    return(
        <button onClick={onClick} className={cn(
            ...
            <Icon className="flex-shrink-0 h-5 w-5 text-indigo-500 dark:text-indigo-400"/>
            <p className={cn(
            ...
            {channel.name ==="general" && (
                <Lock className="ml-auto w-4 h-4 text-zinc-500 hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-300"/>
            )}
        </button>
        )
}